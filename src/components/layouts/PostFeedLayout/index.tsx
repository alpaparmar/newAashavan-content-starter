...
export default function PostFeedLayout(props) {
    ...
    const { title, subtitle, topSections = [], bottomSections = [], pageIndex, baseUrlPath, numOfPages, items, postFeed } = page;
    ...
    return (
        <BaseLayout page={page} site={site}>
            <main id="main" className="layout page-layout">
                ...
                {subtitle && (
                <p
                    className={classNames('text-xl', 'sm:text-2xl', postFeedColors, page?.styles?.subtitle ? mapStyles(page?.styles?.subtitle) : null)}
...
function urlPathForPageAtIndex(pageIndex, baseUrlPath) {
    // 7/3/2023: adding anchor tag to scroll user to the feed when changing pages
    return pageIndex === 0 ? baseUrlPath + '#feed' : `${baseUrlPath}/page/${pageIndex + 1}#feed`;
}
